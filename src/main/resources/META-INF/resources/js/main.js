...
function buildTable() {
  ...
  let randName = generate();
  document.getElementById('playerName').value = randName;
  setCurrentName(randName);
...
function generate() {
  ...
  while (i < 100) {
    ...
    let random2 = Math.floor(Math.random() * NOUNS.length);

    username = toTitleCase(ADJECTIVES[random1], NOUNS[random2]);

    if (used.includes(username)) {
      i++;
    } else {
      used.push(username);
      return username;
...
function toTitleCase(firstname, lastname) {
